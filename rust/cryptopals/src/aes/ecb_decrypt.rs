...
fn encrypt_aes_ecb_suffix_oracle(key: &[u8], plaintext: &[u8]) -> Vec<u8> {
...
fn confirm_aes_ecb(key: &[u8]) {
    let two_blocks = [0u8; 32];
    let ciphertext = encrypt_aes_ecb_suffix_oracle(key, &two_blocks);
    if ciphertext[0..16] != ciphertext[16..32] {
        panic!("expected aes ecb, got ciphertext {:?}", &ciphertext);
    }
}

fn get_aes_ecb_suffix_len(key: &[u8]) -> usize {
    let mut v: Vec<u8> = Vec::new();

    let mut result: usize = 0;
    let start_len = encrypt_aes_ecb_suffix_oracle(key, &v).len();
    for i in 1..17 {
        v.push(0u8);
        let n = encrypt_aes_ecb_suffix_oracle(key, &v).len();
...
fn decrypt_aes_ecb_suffix(key: &[u8]) -> Vec<u8> {
    let n = get_aes_ecb_suffix_len(key);
    ...
    for i in 0..n {
        ...
        for j in 0..256 {
            ...
            let ciphertext = encrypt_aes_ecb_suffix_oracle(key, &block);
            // for some reason "j as u8" returns an Option
            block_map.insert((&ciphertext[0..16]).to_vec(), byte);
        }
        let end = 15 - (i % 16);
        let ciphertext = encrypt_aes_ecb_suffix_oracle(key, &block[0..end]);
...
pub fn decrypt_aes_ecb_simple_test() {
    ...
    confirm_aes_ecb(&key);
    let decrypted_bytes = decrypt_aes_ecb_suffix(&key);
