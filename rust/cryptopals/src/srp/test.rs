...
type SRPPeer = fn (Sender<SRPMsg>, Receiver<SRPMsg>);
...
fn srp_client(tx: Sender<SRPMsg>, rx: Receiver<SRPMsg>) {
    let n = {
        ...
        let tmp = concat!("00c037c37588b4329887e61c2da332",
                          ...
                          "d0d4ca3c500b885fe3");
        Mpz::from_str_radix(tmp, 16).unwrap()
    };
    ...
    tx.send(SRPMsg::Bytes(hmac_sha256(&k, &salt))).unwrap();
}
...
fn srp_server(tx: Sender<SRPMsg>, rx: Receiver<SRPMsg>) {
    ...
    assert_eq!(hmac, client_hmac);
}

fn srp_exchange_test(client: SRPPeer, server: SRPPeer) {
...
pub fn srp_test() {
    ...
    srp_exchange_test(srp_client, srp_server);
